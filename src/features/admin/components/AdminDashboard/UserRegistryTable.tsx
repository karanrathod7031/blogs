    ...
    <div className="bg-card border border-border rounded-[2rem] overflow-hidden shadow-xl">
      <table className="w-full text-left border-collapse">
        ...
        <tbody className="divide-y divide-border">
          {users.map(user => (
            (() => {
              const isRootAuthority = user.email === 'rk.upk2345678@gmail.com';
              const effectiveRole = isRootAuthority ? 'admin' : (user.role || 'user');
                ...
                <tr key={user.uid} className="hover:bg-bg-soft transition-colors">
                  ...
                  <td className="px-8 py-4">
                    <div className="flex gap-2">
                      <span className={`px-2 py-1 rounded-lg border text-[10px] font-black uppercase ${effectiveRole === 'admin' ? 'bg-accent/10 text-accent border-accent/10' : 'bg-bg-soft text-ink-muted border-border'}`}>
                        {isRootAuthority ? 'root admin' : effectiveRole}
                      </span>
                      {user.suspended && (
                  ...
                  <td className="px-8 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button 
                        ...
                        disabled={isRootAuthority || user.uid === currentUserId || suspendingUserIds.has(user.uid)}
                        ...
                        title={isRootAuthority ? 'Root Authority - Immutable' : (user.suspended ? 'Restore Access' : 'Suspend Access')}
                      >
                      ...
                      <button 
                        ...
                        disabled={
                          ...
                          (effectiveRole === 'admin' && users.find(u => u.uid === currentUserId)?.email !== 'rk.upk2345678@gmail.com')
                        }
                        ...
                        title={isRootAuthority ? 'Root Authority - Protected' : 'Purge Identity'}
                      >
