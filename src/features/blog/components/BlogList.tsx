    ...
    >
      ...
      <div className="p-5 md:p-6 space-y-3 md:space-y-4 flex flex-col flex-grow">
        ...
        <h3
          ...
          className="text-lg md:text-xl font-bold leading-tight group-hover:text-accent transition-colors line-clamp-2 cursor-pointer text-ink"
        >
          {post.title}
        </h3>
        ...
        <div
          className="pt-3 border-t border-border/50 flex items-center gap-2 cursor-pointer group/author"
        ...
        >
          ...
          <span className="text-[10px] font-black text-ink-muted group-hover/author:text-accent transition-colors truncate">
            {post.authorName}
          </span>
        </div>
        ...
        <div className="pt-2 flex items-center justify-between text-[10px] font-bold text-ink-muted uppercase tracking-widest">
          <span className="flex items-center gap-1.5">
            <Calendar className="w-3 h-3" /> {format(post.createdAt.toDate(), 'MMM d')}
          </span>
          <span
            onClick={() => onSelectPost(post.slug)}
            className="text-ink font-black group-hover:text-accent transition-colors cursor-pointer"
          >
            Read <ArrowRight className="inline w-3 h-3 ml-1 group-hover:translate-x-0.5 transition-transform" />
          </span>
        </div>
      </div>
    </motion.article>
          ...
          <div className="grid md:grid-cols-2 md:h-[400px] lg:h-[450px]">
            ...
            <div className="p-6 md:p-8 lg:p-12 flex flex-col justify-center space-y-3 md:space-y-6 overflow-hidden">
              <div className="flex gap-3 items-center">
                <span className="badge badge-primary scale-90 md:scale-100 origin-left">
              ...
              <h2
                ...
                className="text-xl md:text-3xl lg:text-4xl font-black leading-tight text-ink group-hover:text-accent transition-colors line-clamp-2 cursor-pointer"
              >
                {featuredPost.title}
              </h2>
              ...
              <div className="pt-2 md:pt-4 flex items-center justify-between gap-2">
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold truncate group-hover/author:text-accent transition-colors text-ink">
                      {featuredPost.authorName}
                    </p>
                    <p className="text-[10px] font-medium text-ink-muted">
                      {format(featuredPost.createdAt.toDate(), 'MMM d, yyyy')}
                    </p>
                  </div>
                </div>
                ...
                <div
                  onClick={() => onSelectPost(featuredPost.slug)}
                  className="flex items-center gap-1.5 md:gap-2 text-accent font-black text-[10px] md:text-sm uppercase tracking-widest whitespace-nowrap shrink-0 cursor-pointer"
                >
      ...
      {!debouncedSearch && activeCategory === 'All' && filteredPosts.length > 0 && (
        <div className="flex flex-col items-center gap-4 pt-8">
          <p className="text-xs font-black uppercase tracking-widest text-ink-muted">
            Page {currentPage}{hasMore ? '' : ` of ${knownPageCount}`}
          </p>
            ...
            {visiblePageNumbers.map((page, index) => {
              ...
              return (
                <div key={page} className="flex items-center gap-2">
                  {showGap && (
                    <span className="px-1 text-sm font-black text-ink-muted">...</span>
                  )}
